...
export function SignupButton({userType}) {
  const { pending } = useFormStatus()
  const buttonText = userType === "staff" ? "Create Staff" : "Sign Up"
  const pendingText = userType === "staff" ? "Creating Staff..." : "Creating Account..."
...
export function AddToCartButton({ foodId, isAvailable, disabled }) {
  ...
  const handleAddToCart = async (e) => {
    ...
    try {
      ...
      const existingItemIndex = existingItems.findIndex((item) => item.food_id === foodId)

      let updatedItems = []
      if (existingItemIndex >= 0) {
        updatedItems = existingItems.map((item) => ({
          food_id: item.food_id,
          quantity: item.food_id === foodId ? item.quantity + 1 : item.quantity,
        }))
      } else {
        updatedItems = [
...
export function CreateFoodButton({ onClick }) {
  return (
    <button onClick={onClick} className={styles.CreateFoodButton}>
      + Create Food
    </button>
  )
}
