...
export default function CartItemCard({ item, onUpdate }) {
  ...
  const updateQuantity = async (newQuantity) => {
    ...
    try {
      ...
      const updatedItems = cartItems.map((cartItem) => ({
        food_id: cartItem.food.id, // Access the food ID from the nested food object
  ...
  const itemTotal = (Number.parseFloat(item.food.price) * quantity).toFixed(2)

  return (
    <div className={styles.card}>
      <div className={styles.content}>
        <div className={styles.info}>
          <h3 className={styles.name}>{item.food.name}</h3>
          <p className={styles.price}>${item.food.price} each</p>
        </div>
